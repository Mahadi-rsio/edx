...
const Test: React.FC = () => {
  ...
  const fetchKeywords = async () => {
    ...
    try {
      ...
      const fetchedOptions: Option[] = data.map((item) => ({ label: item.label }));
  ...
  const saveKeyword = async (keyword: string) => {
    try {
      ...
      setOptions((prev) => [...prev, { label: keyword }].sort((a, b) =>
  ...
  const handleSubmit = async () => {
    if (inputValue.trim()) {
      ...
      const keywordExists = options.some(
        (option) => option.label.toLowerCase() === keyword.toLowerCase()
      );
      ...
      <StyledAutocomplete
        ...
        renderInput={(params) => (
          <TextField
            ...
            InputProps={{
              ...
              endAdornment: (
                <FaSearch
                  style={{ marginLeft: '8px', color: '#757575', cursor: 'pointer' }}
        ...
        renderOption={(props, option) => (
          <li {...props} style={{ display: 'flex', alignItems: 'center' }}>
            <FaSearch style={{ marginRight: '8px', color: '#757575' }} />
            {option.label}
          </li>
        )}
