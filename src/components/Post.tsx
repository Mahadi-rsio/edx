...
const Post: React.FC<FacebookPostProps> = ({
  ...
  return (
    ...
    >
        ...
        <Box
          sx={{
            ...
            '&::-webkit-scrollbar-thumb': {
              backgroundColor: isContentLong ? theme.palette.primary.main : 'transparent',
              borderRadius: '4px',
            },
          }}
        >
        ...
        {hashtags.length > 0 && (
          <Stack direction="row" spacing={1} mt={2} flexWrap="wrap">
            {hashtags.map((tag, index) => (
              <Chip key={index} label={`#${tag}`} variant='outlined' clickable color="primary" size="small" />
            ))}
          </Stack>
        )}
      </CardContent>
      ...
      >
        <IconButton aria-label="like post" sx={{ display: 'flex', alignItems: 'center' }}>
          <BsHeart size={20} />
          <Typography variant="caption" sx={{ ml: 0.5 }}>
            {likeCount}
          </Typography>
        </IconButton>
        <IconButton aria-label="comment on post" sx={{ display: 'flex', alignItems: 'center' }}>
          <BsChat size={20} />
          <Typography variant="caption" sx={{ ml: 0.5 }}>
            {commentCount}
          </Typography>
        </IconButton>
        <IconButton aria-label="share post" sx={{ display: 'flex', alignItems: 'center' }}>
          <BsShare size={20} />
          <Typography variant="caption" sx={{ ml: 0.5 }}>
            {shareCount}
          </Typography>
